...
- **最优批次匹配**：**用最少的批次条数**凑出目标库存。优先尝试整批次精确组合（0/1 背包），无解时支持"切件"——最后一条批次只领用部分库存（如批次有 10 件，只需 5 件即拨 5 件，剩 5 件留库）。
- **结果一键复制**：
  - 复制批次号列
  - 复制库存列（复制后弹窗提示是否同步从数据库扣减领用量）
  - 仅删除记录（按本次领用量扣减；全量领用的批次直接删除）
...
5. **删除已用记录**（可选）：点"复制库存列"后会弹出"是否删除记录"提示，或单独点"仅删除记录"。被全量领用的批次会从数据库删除；切件领用的批次只会从原库存中扣减领用量。
...
- 目标：在某型号的全部批次记录中，**用最少的批次条数凑出目标库存**。每条批次记录最多使用一次，且最后一条允许"切件"（仅领用部分库存）。
- 策略：
  1. 先按库存量降序贪心选取批次，得到允许切件下的最少条数 `k_partial`（最后一条的领用量 = 目标值 − 前面整批之和）。
  2. 再用 0/1 背包 DP 检查是否存在 `k_partial` 条**整批次**精确凑出目标值的子集。若存在则采用整批方案（不切件，数据更干净）；否则采用第 1 步的切件方案。
- 上限：单次匹配最多考虑前 300 条候选记录。
- 若所有批次库存之和小于目标值，或目标值非正整数，会提示"无法凑出该数量"。
